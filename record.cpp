...
void Record::logActivity(string activity) {
    ...
    activities.push_back(timestampedActivity);
}

void Record::logAttendance(string at){
    string timestampedAttendence = getCurrentTime() + "\t\t\t" + at;
    attendance.push_back(timestampedAttendence);
}
...
void Record::saveToFile(string filename) {
    ...
    try {
        ...
        for (const auto& activity : activities) {
            Writer << activity << endl;
        }
        
        Writer << "attendance\n";

        for (const auto& at : attendance) {
            Writer << at << endl;
        }
    }
    catch (exception& e) {
    ...
    try {
        ...
        while (getline(reader, read)) {
            if (read == "attendance") {
                ...
                activities.push_back(read);
            }
        }

        while (getline(reader, read)) {
            attendance.push_back(read);
        }
